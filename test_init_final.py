...
def init():
    ...
    for j in range(fixed_bossNum):
        ...
        fixed_bossTime.append(tmp_fixed_now.replace(year=int(fixed_bossData[j][7]), month=int(fixed_bossData[j][8]),
                                                    day=int(fixed_bossData[j][9]), hour=int(fixed_bossData[j][1]),
                                                    minute=int(fixed_bossData[j][2]), second=int(0)))
        if fixed_bossTime[j] < tmp_fixed_now:
            while fixed_bossTime[j] < tmp_fixed_now:
                fixed_bossTime[j] = fixed_bossTime[j] + datetime.timedelta(hours=int(fixed_bossData[j][5]),
                                                                           minutes=int(fixed_bossData[j][6]),
                                                                           seconds=int(0))
    ...
    for i in range(len(regenTime)):
        if outputTimeMin[i] == 0:
            regenembed.add_field(name=str(outputTimeHour[i]) + '시간',
                                 value='```' + ', '.join(map(str, sorted(regenbossName[i]))) + '```', inline=False)
        else:
            regenembed.add_field(name=str(outputTimeHour[i]) + '시간' + str(outputTimeMin[i]) + '분',
                                 value='```' + ','.join(map(str, sorted(regenbossName[i]))) + '```', inline=False)
...
async def task():
    ...
    while not client.is_closed():
        ...
        if channel != '':
            ################ 보탐봇 재시작 ################
            if endTime.strftime('%Y-%m-%d ') + endTime.strftime('%H:%M:%S') == now.strftime('%Y-%m-%d ') + now.strftime(
                    '%H:%M:%S'):
                if basicSetting[2] != '0':
                    for i in range(bossNum):
                        if bossMungFlag[i] == True:
                    ...
                    if basicSetting[3] != '0':
                        if fixed_bossFlag0[i] == False:
                            ...
                            await client.get_channel(channel).send(
                                "```" + fixed_bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + fixed_bossData[i][
                                    3] + ' [' + fixed_bossTime[i].strftime('%H:%M:%S') + ']```', tts=False)
                ...
                if fixed_bossTime[i] <= priv and fixed_bossTime[i] > now:
                    if basicSetting[1] != '0':
                        if fixed_bossFlag[i] == False:
                            ...
                            await client.get_channel(channel).send(
                                "```" + fixed_bossData[i][0] + ' ' + basicSetting[1] + '분 전 ' + fixed_bossData[i][
                                    3] + ' [' + fixed_bossTime[i].strftime('%H:%M:%S') + ']```', tts=False)
                            await PlaySound(voice_client1, './sound/' + fixed_bossData[i][0] + '알림.mp3')

                ################ 보스 젠 시간 확인 ################
                if fixed_bossTime[i] <= now:
                    fixed_bossTime[i] = fixed_bossTime[i] + datetime.timedelta(hours=int(fixed_bossData[i][5]),
                                                                               minutes=int(fixed_bossData[i][6]),
                                                                               seconds=int(0))
                ...
                if bossTime[i] <= priv0 and bossTime[i] > priv:
                    if basicSetting[3] != '0':
                        if bossFlag0[i] == False:
                            ...
                            if bossData[i][6] != '':
                                await client.get_channel(channel).send(
                                    "```" + bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + bossData[i][3] + " [" +
                                    bossTimeString[i] + "]" + '\n<' + bossData[i][6] + '>```', tts=False)
                            else:
                                await client.get_channel(channel).send(
                                    "```" + bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + bossData[i][3] + " [" +
                                    bossTimeString[i] + "]```", tts=False)
                    ...
                    if (bossTime[i] + datetime.timedelta(days=-365)) <= aftr:
                        if basicSetting[2] != '0':
                            ...
                            if bossData[i][2] == '0':
                                ...
                                tmp_bossTime[i] = bossTime[i] = nextTime = tmp_bossTime[i] + datetime.timedelta(
                                    hours=int(bossData[i][1]), minutes=int(bossData[i][5]))
...
async def MakeSound(saveSTR, filename):
    ...
    try:
        ...
        urllib.request.urlretrieve(
            "https://clova.ai/proxy/voice/api/tts?text=" + encText + "%0A&voicefont=1&format=wav", filename + '.wav')
        ...
        for i in range(bossNum):
            if timestring == bossTime[i]:
                if bossTimeString[i] != '99:99:99':
                    if bossData[i][2] == '0':
                        information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + \
                                        bossTimeString[i] + ' @ ' + bossDateString[i] + ' (미입력 ' + str(
                            bossMungCnt[i]) + '회)' + ' * ' + bossData[i][6] + '\n'
                    else:
                        information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + \
                                        bossTimeString[i] + ' @ ' + bossDateString[i] + ' (멍 ' + str(
                            bossMungCnt[i]) + '회)' + ' * ' + bossData[i][6] + '\n'
        ...
        for i in range(len(beforeBossData) - 1):
            for j in range(bossNum):
                ...
                if beforeBossData[i + 1][startPos + 2:endPos] == bossData[j][0]:
                    ...
                    tmp_now = datetime.datetime.now() + datetime.timedelta(hours=int(basicSetting[0]))
                    tmp_now = tmp_now.replace(year=int(years1), month=int(months1), day=int(days1), hour=int(hours1),
                                              minute=int(minutes1), second=int(seconds1))
                    ...
                    if beforeBossData[i + 1][tmp_msglen - 4:tmp_msglen - 3] != 0 and beforeBossData[i + 1][
                                                                                     tmp_msglen - 5:tmp_msglen - 4] == ' ':
                        ...
                        bossMungCnt[j] = int(
                            beforeBossData[i + 1][tmp_msglen - 5:tmp_msglen - 4] + beforeBossData[i + 1][
                                                                                   tmp_msglen - 4:tmp_msglen - 3]) + tmp_mungcnt
                    else:
                        bossMungCnt[j] = 0
        LoadChk = 0
        print("<불러오기 완료>")
...
async def on_ready():
    ...
    await client.change_presence(status=discord.Status.dnd, activity=discord.Game(name="재고현황 안내", type=1), afk=False)
...
while True:
    ...
    async def on_message(msg):
        ...
        if client.get_channel(channel) != msg.channel:
            ...
            if basicSetting[11] != "":
                if msg.channel.id == int(basicSetting[11]):  #### 정산채널 채널ID 값넣으면 됨
                    ...
                    if message.content.startswith(command[12]):
                        if basicSetting[10] != "" and basicSetting[12] != "" and basicSetting[14] != "" and \
                                basicSetting[15] != "" and basicSetting[16] != "":
            ...
            if message.content.startswith(command[22]):
                if basicSetting[10] != "" and basicSetting[12] != "" and basicSetting[17] != "" and basicSetting[
                    18] != "" and basicSetting[19] != "":
    ...
    for i in range(bossNum):
        ...
        if message.content.startswith(bossData[i][0] + '멍'):
            ...
            if len(hello) > len(tmp_msg) + 3:
                ...
                if tmp_bossTime[i] < tmp_now:
                    ...
                    nextTime = tmp_bossTime[i] + datetime.timedelta(hours=int(bossData[i][1]),
                                                                    minutes=int(bossData[i][5]))
                    ...
                    await client.get_channel(channel).send(
                        '```' + bossData[i][0] + '탐이 아직 안됐습니다. 다음 ' + bossData[i][0] + '탐 [' + tmp_bossTimeString[
                            i] + '] 입니다```', tts=False)
        ...
        if message.content.startswith(bossData[i][0] + '메모 '):
            ...
            await client.get_channel(channel).send('< ' + bossData[i][0] + ' [ ' + bossData[i][6] + ' ] 메모등록 완료>',
                                                   tts=False)
        ...
        if num_sep == 0:
            ...
            await client.get_channel(channel).send(
                '```1차세금 : ' + str(cal_tax1) + '\n1차 수령액 : ' + str(real_money) + '\n분배자 거래소등록금액 : ' + str(
                    int(real_money / num_sep)) + '\n2차세금 : ' + str(cal_tax2) + '\n인당 실수령액 : ' + str(
                    int(float(int(real_money / num_sep)) * 0.95)) + '```', tts=False)
    ...
    if message.content.startswith(command[15]) or message.content.startswith('!ㅍ') or message.content.startswith('!V'):
        ...
        await client.get_channel(channel).send("```< " + msg.author.display_name + " >님이 \"" + sayMessage + "\"```",
                                               tts=False)
    ...
    if message.content == command[4]:
        if message.author.voice == None:
            ...
            await client.get_channel(channel).send('< 음성채널 [' + client.get_channel(voice_channel.id).name + '] 접속완료>',
                                                   tts=False)
                ...
                for j in range(int(basicSetting[9])):
                    for i in range(len(ouput_bossData)):
                        if sorted_datelist[j] == ouput_bossData[i][1]:
                            leftTime = ouput_bossData[i][1] - (
                                        datetime.datetime.now() + datetime.timedelta(hours=int(basicSetting[0])))

                            total_seconds = int(leftTime.total_seconds())
                            hours, remainder = divmod(total_seconds, 60 * 60)
                            minutes, seconds = divmod(remainder, 60)

                            result_lefttime += '다음 ' + ouput_bossData[i][0] + '탐까지 %02d:%02d:%02d 남았습니다. ' % (
                            hours, minutes, seconds) + '[' + ouput_bossData[i][2] + ']\n'
            else:
                ...
                if timestring == ouput_bossData[i][1]:
                    if ouput_bossData[i][4] == '0':
                        if ouput_bossData[i][5] == 0:
                            information += ouput_bossData[i][3] + ' ' + ouput_bossData[i][2] + ' : ' + \
                                           ouput_bossData[i][0] + ' ' + ouput_bossData[i][6] + '\n'
                        else:
                            information += ouput_bossData[i][3] + ' ' + ouput_bossData[i][2] + ' : ' + \
                                           ouput_bossData[i][0] + ' (미 ' + str(ouput_bossData[i][5]) + '회)' + ' ' + \
                                           ouput_bossData[i][6] + '\n'
                    else:
                        if ouput_bossData[i][5] == 0:
                            information += ouput_bossData[i][3] + ' ' + ouput_bossData[i][2] + ' : ' + \
                            ...
                            information += ouput_bossData[i][3] + ' ' + ouput_bossData[i][2] + ' : ' + \
                                           ouput_bossData[i][0] + ' (멍 ' + str(ouput_bossData[i][5]) + '회)' + ' ' + \
                                           ouput_bossData[i][6] + '\n'
        ...
        for timestring1 in sorted(fixed_datelist):
            for i in range(fixed_bossNum):
                if timestring1 == fixed_bossTime[i]:
                    if (datetime.datetime.now() + datetime.timedelta(hours=int(basicSetting[0]))).strftime(
                            '%Y-%m-%d') == fixed_bossTime[i].strftime('%Y-%m-%d'):
                        tmp_timeSTR = fixed_bossTime[i].strftime('%H:%M:%S')
                    else:
                        tmp_timeSTR = '[' + fixed_bossTime[i].strftime('%Y-%m-%d') + '] ' + fixed_bossTime[i].strftime(
                            '%H:%M:%S')
    ...
    if message.content == command[17]:
        ...
        embed = discord.Embed(
            title='현재시간은 ' + curruntTime.strftime('%H') + '시 ' + curruntTime.strftime(
                '%M') + '분 ' + curruntTime.strftime('%S') + '초 입니다.',
            color=0xff00ff
        )
        await client.get_channel(channel).send(embed=embed, tts=False)
    ...
    if message.content.startswith(command[19]):
        ...
        await client.change_presence(status=discord.Status.dnd, activity=discord.Game(name=sayMessage, type=1),
                                     afk=False)
    ...
    if message.content.startswith(command[12]):
        if basicSetting[10] != "" and basicSetting[12] != "" and basicSetting[14] != "" and basicSetting[15] != "" and \
                basicSetting[16] != "":
